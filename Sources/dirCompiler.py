...
def compileProgram(dir_name: str, exec_name: str, files_to_execute: list):
    ...
    with open("Objects/__entry.s", "w") as entry_file:
        entry_file.write(generateEntry(files_to_execute))
    compileAssembly("Objects/__entry.s", "Objects/__entry.o")
    object_names.append("Objects/__entry.o")
...
def compileDir(dir_name: str):
    if not dir_name[-1] == "/":
        dir_name += "/"
    if path.isfile(f"{dir_name}Info.json"):
        if not path.isdir("Sources"):
